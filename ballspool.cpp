...
void BallsPool::update(Map *m_map, double time_left, BonusController &bonus_controller)
{
    ...
    while (iter_pool.hasNext())
    {
        ...
        for(Brick *brick : m_map->map)
                ...
                || ball->m_area->contains(*brick->m_area))
            {
                ...
                brick->decreasePower();
                bonus_controller.generateBonus();
            }
